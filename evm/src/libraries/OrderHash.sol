...
library OrderHash {
    ...
    /// @notice The separator those three constants hash to. Pinned as a literal so every digest is one
    ///         `mload`, not three keccaks; `OrderHash.t.sol` recomputes it from the parts.
    bytes32 internal constant DOMAIN_SEPARATOR = 0x30be5c0eff57121410bcfb088c479028b3b8059112c22dff7e7b55f1b3927d7d;
    ...
    /// @notice The minimal EIP-712 domain separator.
    function domainSeparator() internal pure returns (bytes32) {
        return DOMAIN_SEPARATOR;
    }

    /// @dev What {DOMAIN_SEPARATOR} is pinned to; the test asserts the two agree.
    function computeDomainSeparator() internal pure returns (bytes32) {
