...
# Build noir circuits and generate UltraHonk proofs/vk for tests
# Based on ultrahonk_soroban_contract/tests/build_circuits.sh
...
for dir in */ ; do
  ...
  # Generate proof with keccak oracle hash (for external verification)
  ...
  # Flatten the vk directory structure (bb creates target/vk/vk, we want target/vk)
  ...
  # Also flatten proof directory if needed
...
find . -type f -name "vk" -o -name "proof" -o -name "public_inputs" 2>/dev/null | sort
