...
/// #453: the longest `deadline - now` a lock/create accepts, and so the longest `min_window`.
...
/// `margin <= min_window <= MAX_ORDER_WINDOW` (the upper bounds are #453's).
pub fn validate(t: &RouteTiming) -> Result<(), u8> {
    ...
    if t.min_window < t.margin || t.min_window > MAX_ORDER_WINDOW {
        return Err(5);
    }
    Ok(())
}
